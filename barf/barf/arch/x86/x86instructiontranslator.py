...
def infer_operands_size(instr, arch_size):
    if instr.mnemonic in [  ReilMnemonic.ADD, ReilMnemonic.SUB,
        ...
        # print "target size: ", instr.operands[0].size, arch_size, str(instr)

        assert instr.operands[0].size == arch_size #, "instr: %s" % str(instr)
...
def check_instruction(instruction):
    if instruction.mnemonic == "mov":
        ...
        oprnd1 = instruction.operands[0]
        oprnd2 = instruction.operands[1]

        assert oprnd1.size and oprnd2.size
        assert oprnd1.size == oprnd2.size

    elif instruction.mnemonic == "movzx":
    ...
    elif instruction.mnemonic == "xchg":
...
class X86InstructionTranslator(object):
    ...
    def translate(self, instruction):
        ...
        tb = TranslationBuilder(self.ir_name_generator, self.architecture_mode)
